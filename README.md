...
3. Configure usuário de banco com permissões de leitura/escrita
4. Adicione `0.0.0.0/0` na whitelist de IPs
...
1. **Coleta** → Sensor DHT22 (simulado) coleta temperatura e umidade
2. **Limpeza** → Validação de ranges e detecção de anomalias
3. **Armazenamento** → Persistência em MongoDB Atlas
4. **Monitoramento** → Estatísticas em tempo real
...
```python
from core.interfaces import IDataCleaner
